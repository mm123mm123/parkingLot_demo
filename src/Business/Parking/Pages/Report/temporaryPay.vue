  ...
  methods: {
    ...
    handleExport () {
      this.fetchListData(1, 9999).then(res => {
        ...
        this.$refs.table.exportXlsx('临时缴费明细表', tableData)
      })
    },
    handleRow (data) {
      console.log(data)
    ...
    fetchListData (page_no, page_size) {
      ...
      return this.$https.post(this.$api.parking.report.get_temporary_list, params).then(res => {
      ...
      if (res.code === 1000 && res?.list) {
        ...
        tableData = res.list.map(x => {
          ...
          let item = {
            ...
            duration_text,
            paid_channel_text,
            actual_share_pay: actual_share_pay / 100
          }
          return item
        })

      ...
      tableLabel: [
        ...
        },
        {
          ...
          children: [
            ...
            {
              prop: 'bill_offline_paid_type4',
              label: '线下(元)'
            },
            {
              prop: 'bill_offline_rate',
              label: '支付占比'
            },
          ]
        },
  ...
  methods: {
    ...
    handleExport () {
      this.fetchListData(1, 9999).then(res => {
        ...
        this.$refs.table.exportXlsx('催缴支付情况统计表', tableData)
      })
    },
    handleRow (data) {
      console.log(data)
    ...
    fetchListData (page_no, page_size) {
      ...
      return this.$https.post(this.$api.parking.report.get_reminder_pay_sum_list, params).then(res => {
    ...
    handleData (res) {
      ...
      if (res.code === 1000 && res?.list) {
        ...
        tableData = res.list.map(x => {
          const bill_sms_rate = !x.little_money ? '-' : Math.round(((x.bill_sms_paid_type / x.little_money) || 0) * 1000) / 10 + '%'
          const bill_offline_rate = !x.little_money ? '-' : Math.round(((x.bill_offline_paid_type4 / x.little_money) || 0) * 1000) / 10 + '%'
          const paid_rate = !x.count_money ? null : Math.round(((x.little_money / x.count_money) || 0) * 1000) / 10 + '%'
          const {
            bill_offline_paid_type4 = 0,
            bill_sms_paid_type = 0,
            count_money = 0,
            little_money = 0,
            need_paid = 0,
            should_pay = 0,
          ...
          let item = {
            ...
            bill_offline_paid_type4: bill_offline_paid_type4 / 100,
            bill_sms_paid_type: bill_sms_paid_type / 100,
